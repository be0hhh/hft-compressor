...
bool parseRows(std::span<const std::uint8_t> input, std::vector<Row>& rows) {
    std::int64_t prevTs = 0; bool have = false; std::size_t start = 0;
    while (start < input.size()) {
        std::size_t end = start; while (end < input.size() && input[end] != static_cast<std::uint8_t>('\n')) ++end;
        std::string_view line{reinterpret_cast<const char*>(input.data() + start), end - start};
        if (!line.empty() && line.back() == '\r') line.remove_suffix(1);
        if (line.empty()) return false;
        Row row{}; if (!parseLine(line, row)) return false;
        if (have && row.ts < prevTs) return false;
        prevTs = row.ts; have = true; rows.push_back(row);
...
bool readHeader(std::span<const std::uint8_t> data, Header& h) noexcept {
    ...
    return rd(p,e,h.magic) && rd(p,e,h.version) && rd(p,e,h.reserved) && rd(p,e,h.inputBytes) && rd(p,e,h.outputBytes) && rd(p,e,h.recordCount)
        ...
        && h.magic == kMagic && h.version == kVersion && h.timeScale != 0 && h.priceScale != 0 && h.qtyScale != 0;
}
...
Status decodeBytes(std::span<const std::uint8_t> data, std::string* jsonl, std::ostream* encoded) noexcept {
    ...
    for (std::uint64_t i=0; i<h.recordCount; ++i) {
        ...
        if (i != 0) {
            if (!readVar(tp,te,dt) || mp>=me) return Status::CorruptData; ts += static_cast<std::int64_t>(dt); m = *mp++;
            std::uint64_t v=0;
    ...
    if (tp!=te || mp!=me || bp!=be || sp!=se || bqp!=bqe || aqp!=aqe) return Status::CorruptData;
    if (encoded) *encoded << "\n  ]\n}\n";
    return Status::Ok;
}
...
Status inspectEncodedBinaryFile(const std::filesystem::path& path, const DecodedBlockCallback& cb) noexcept { std::vector<std::uint8_t> data; if(!readFile(path,data)) return Status::IoError; std::ostringstream o; Header h{}; if(!readHeader(data,h)) return Status::CorruptData; o << "bookticker_delta_mask_v1 bytes=" << data.size() << " header=" << kHeaderBytes << " time=" << h.timeBytes << " mask=" << h.maskBytes << " bid=" << h.bidBytes << " spread=" << h.spreadBytes << " bid_qty=" << h.bidQtyBytes << " ask_qty=" << h.askQtyBytes << "\n"; return emitText(o.str(),cb); }
